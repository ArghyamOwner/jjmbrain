<div>
    <x-card no-padding overflow-hidden>
        <x-slot name="header" class="border-b">
            <x-heading size="lg">Class Milestones</x-heading>
            <x-slot name="subheading">
                <p class="text-slate-500 text-sm">All milestones associated with a class.</p>
            </x-slot>
                        ...
                        <tr>
                            ...
                            <x-table.tdata>
                                <div class="flex">
                                    <x-button-icon-edit 
                                        href="#" 
                                        x-on:click.prevent="livewire.emitTo('classes.edit-milestone','showEditMilestoneSlideover', '{{ $milestone->id }}')" 
                                    />
                                </div>
                            </x-table.tdata>
                        </tr>
    ...
    <livewire:classes.edit-milestone />

    <x-modal-simple max-width="md" name="milestone-create-form" form-action="saveClassMilestone">
        <x-slot name="title">Create new class milestone</x-slot>

        <x-input label="Milestone Title" name="milestoneTitle" wire:model.defer="milestoneTitle" />
         
        <x-slot name="footer">
            <x-button type="submit" with-spinner wire:target="saveClassMilestone">Save</x-button>
        </x-slot>
    </x-modal-simple>
</div>
