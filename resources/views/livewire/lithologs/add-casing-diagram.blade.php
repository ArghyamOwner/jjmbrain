<div>
    <div class="text-right p-1">
        {{-- <x-button class="truncate" tag="a" href="#" color="white" with-icon icon="add" x-data="{}"
            x-on:click.prevent="$dispatch('show-modal', 'casing-diagram-form')" x-cloak>
            Add
        </x-button> --}}

        <x-button-icon-add href="#" x-on:click.prevent="$dispatch('show-modal', 'casing-diagram-form')" />
    </div>

    <x-modal-simple max-width="2xl" name="casing-diagram-form" form-action="save">
        <x-slot name="title">Add Casing Diagram Details to Litholog</x-slot>


        <div class="grid grid-cols-1 md:grid-cols-2 gap-x-6">
            <x-select label="Latern" name="pattern_id" wire:model.defer="pattern_id">
