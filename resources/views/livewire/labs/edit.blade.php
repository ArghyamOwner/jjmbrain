<div>
    <x-slot name="title">Edit Labs</x-slot>
        ...
        <x-card overflow-hidden form-action="save">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    @if(is_null($this->documentCheck))
                    <x-filepond label="Upload NABL Certification" name="document" wire:model.defer="document"
                        hint="Maximum file size: 2 MB. Allowed file type: PDF" accept-files="application/pdf"
                        optional />
                    @endif

                    <x-input type="date" label="NABL Certification Expiry Date" name="expiryDate"
                        wire:model.defer="expiryDate" optional />

                    <x-radio-pill label="NABL Certification" name="nablCertification" wire:model="nablCertification"
                        :default-value="$nablCertification" :options="[
                            [
                                'label' => 'Yes',
                                'value' => 'yes',
                            ],
