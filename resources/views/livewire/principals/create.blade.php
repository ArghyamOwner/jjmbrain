<div>
    <x-slot name="title">Create Principal</x-slot>
 
    <x-slot name="secondaryTopbar">
        <x-navbar-top-transparent>
            <x-slot:beforeTitle>
                <x-text-link with-back-icon href="{{ route('principals') }}">Go Back</x-text-link>
            </x-slot>
    ...
    <x-section-centered>
        <x-card overflow-hidden form-action="save">
            <x-card-form :with-shadow="false" no-padding>
                <x-slot name="title">Principal Details</x-slot>
                <x-slot name="description">Add the necessary details of a principal.</x-slot>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <x-input label="Principal ID" name="principalId" wire:model.defer="principalId" />
