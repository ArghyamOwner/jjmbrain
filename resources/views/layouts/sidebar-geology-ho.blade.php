...
<x-sidebar>
	...
	<x-nav-item to="{{ route('schemes', ['has_litholog'=> 'yes']) }}" class="flex items-center py-2">
		<x-icon-folder class="mr-4 text-slate-400 w-5 h-5" />Schemes with Litholog
	</x-nav-item>

	<x-nav-item to="{{ route('schemes', ['without_sdo_approval'=> 'yes']) }}" class="flex items-center py-2">
		<x-icon-folder class="mr-4 text-slate-400 w-5 h-5" />Schemes for Litholog (SDO Approval)
	</x-nav-item>

	<x-nav-item to="{{ route('schemes', ['tracking'=> 'yes']) }}" class="flex items-center py-2">
		<x-icon-folder class="mr-4 text-slate-400 w-5 h-5" />Schemes (Map Tracked)
	</x-nav-item>
	...
	{{-- <x-nav-item to="{{ route('lithologs.heatMap') }}" class="flex items-center py-2">
		<x-icon-map class="mr-4 text-slate-400 w-5 h-5"/>Litholog Heat Map
	</x-nav-item> --}}
