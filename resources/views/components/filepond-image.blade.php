...
<div class="{{ $noMargin ? 'mb-0' : 'mb-5' }}">
	...
	<div
		...
		wire:ignore
		wire:key="filepond-{{ md5($attributes->wire('model')->value) }}"
		x-data="{ 
			pond: null,
			hasError: false,
			errorText: '',
				...
				pond.setOptions({
					...
					server: {
						process: (fieldname, file, metaData, load, error, progress, abort, transfer, options) => {
							{{-- @this.uploadMultiple('{{ $attributes->wire('model')->value }}', [file], load, error, progress) --}}

							const compressor = new Compressor(file, {
								quality: imageQuality, // Adjust the compression quality as needed
								success: (compressedFile) => {
									// console.log(compressedFile)
									@this.upload('{{ $attributes->wire('model')->value }}', compressedFile, load, error, progress)
								},
								error: (err) => {
								...
								drew(context, canvas) {
									const uploadedDate = `{{ now()->format('d/m/Y h:i a') }}`;

									context.fillStyle = '#fff';
									context.font = '4rem serif';
									context.fillText('Jal Jeevan Mission Portal', 40, canvas.height - 140);
									context.fillText(uploadedDate, 40, canvas.height - 60);
								},
							});
